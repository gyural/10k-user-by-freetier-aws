...
public class ParkingHistoryRepositoryTest {
    ...
    void updateDeleteAtByIds_ShouldUpdateDeleteAt_WhenIdsExist() {
        ...
        parkingHistoryRepository.updateDeleteAtByIds(now, List.of(history2.getId()));

        entityManager.flush(); // 즉시 반영
        entityManager.clear(); // 영속성 컨텍스트 초기화

        // Then
        ParkingHistoryEntity nonDeletedHistory1 = parkingHistoryRepository.findById(history1.getId()).orElse(null);
        ParkingHistoryEntity deleteHistory = parkingHistoryRepository.findById(history2.getId()).orElse(null);
    ...
    void findByIdsAndMember_ShouldReturnHistories_WhenIdsAndMemberMatch() {
        ...
        // When
        List<ParkingHistoryEntity> result = parkingHistoryRepository.findByIdsAndMember(List.of(history1.getId(), history2.getId()), memberId);

        // Then
        assertNotNull(result);
        assertEquals(2, result.size());
        assertTrue(result.stream().allMatch(history -> history.getMemberEntity().getId().equals(memberId)));
    }
    ...
    void deleteAllByDeleteAtBefore_ShouldDeleteRecords_WhenMatchingRecordsExist() {
        ...
        ParkingHistoryEntity oldHistory1 = parkingHistoryRepository.findById(history1.getId()).orElse(null);
        ParkingHistoryEntity newHistory = parkingHistoryRepository.findById(history2.getId()).orElse(null);
        ParkingHistoryEntity oldHistory2 = parkingHistoryRepository.findById(preSoftDeletedHistory.getId()).orElse(null);
